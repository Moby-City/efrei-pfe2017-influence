...
target_words
save(target_words, file="target_words.rda")
...
saveRDS(dtMatrix,file="dtMatrix.rds")

# configure training data
container <- create_container(dtMatrix, df2$y, trainSize=1:15000, virgin=FALSE)
# train a SVM Model
model <- train_model(container, "SVM", kernel="linear", cost=1)
saveRDS(model,file="SVM_Model.rds")

# create data for prediction based on what was not used for training
predictionData = df2[19100,1]
predictionData_trueTarget = df2[19100,2]
